# Amazon Q CLIで作るStrands Agent活用のAWSアーキテクチャ学習ゲーム開発記

## はじめに

Amazon Q CLIを使って、Strands AgentベースのAWSアーキテクチャ学習ゲームを開発した体験を記録します。この記事では、実際の開発過程で起こった試行錯誤、問題解決、そして最終的な成果物について、Amazon Q CLIとの対話を通じて進めた開発の全貌をお伝えします。

## 🎯 プロジェクトの始まり

### Amazon Q CLIでの相談

最初に私がAmazon Q CLIに相談したのは、「Strands Agent を使ったゲームを考えたいです。初めて使うのでまずは調査からしてください。その後に、面白いネタを３つ考えてください。AWSリソースに関連するものがいいです。」という内容でした。
...
私から「ユーザー名が必須になるようにして下さい。ユーザーの入力値で提案以外のものが出たら、Yes noで答えるように応答して下さい」という要求を出しました。
...
**プロジェクトリポジトリ**: `/home/atom/aws_architecture_quiz/`
